...
class DraggableMascot(FloatLayout):
    def __init__(self, **kwargs):
        ...
        self.pos = (dp(40), dp(900))
        
        # 1. On lie le fond graphique aux coordonnées exactes (self.x, self.y) du conteneur
        with self.canvas.before:
            Color(0.08, 0.18, 0.32, 0.05)
            self.bg_rect = RoundedRectangle(size=self.size, pos=self.pos, radius=[8])
        
        # On s'assure que le fond suit le conteneur si on le déplace
        self.bind(pos=self._update_graphics, size=self._update_graphics)

        # 2. Conteneur interne positionné exactement sur la taille 
        ...
        self.bulle_label = Label(
            text="glissesur sur les messages pour un déplacement",
            font_size='12sp',
            color=(1, 1, 1, 1),
            halign='center',
            valign='middle',
    ...
    def changer_message_aleatoire(self, dt):
        messages = [
            ...
            "Montre ton génie !",
            "Glisse sur les messages pour me déplacé!",
    ...
    def mettre_a_jour_progression(self, grade_niveau):
        if grade_niveau >= 5:
            self.avatar_label.text = "[b]>> LOCKY (PRO) <<[/b]"
            self.avatar_label.color = (0, 0.7, 1, 1)
        elif grade_niveau >= 3:
            self.avatar_label.text = "[b]>> LOCKY (ELITE) <<[/b]"
            self.avatar_label.color = (0.3, 0.9, 1, 1)
        else:
            self.avatar_label.text = "[b]>> LOCKY <<[/b]"
            self.avatar_label.color = (1, 0.84, 0, 1)
...
class FixedDynamicQuizApp(App):
    ...
    def sequence_intro_locky(self, mascot):
        ...
        points = [
            ...
            (Window.width * 0.6, Window.height * 0.1),
    ...
    def aller_au_menu_principal(self, ecran_intro):
        ...
        if self.mascotte not in self.main_layout.children:
            self.main_layout.add_widget(self.mascotte)
